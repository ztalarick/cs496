...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  ...
  | None -> failwith "lookup_class: class c_name not found"
  | Some(s, f, m) -> return (s, f, m)
   
let new_env : string list -> env ea_result = fun fs ->
  let rec new_envh = fun fs ->
    match fs with
    | [] -> lookup_env
    | x::xs -> let l = Store.new_ref g_store (NumVal 0)
      in extend_env x (RefVal l) >>+ new_envh xs
  in return EmptyEnv >>+ new_envh fs
...
let lookup_method : string -> string -> class_env -> method_decl option = fun c_name m_name c_env ->
    match List.assoc_opt c_name c_env with
    | None -> None
    | Some (_,_,m) -> List.assoc_opt m_name m
...
and
  ...
  (* SOOL operations *)
  | NewObject(c_name,es) ->
    sequence (List.map eval_expr es) >>= fun args -> lookup_class c_name !g_class_env
    >>= fun (_, f, me) -> new_env f >>= fun env ->
    let self = ObjectVal(c_name, env)
    in (match (List.assoc_opt "initialize" me) with
    | None -> return self
    | Some m -> apply_method "initialize" self args m>>= fun _ ->
      return self)
  | Send(e,m_name,es) ->  eval_expr e >>= fun self ->
  ...
  | Self ->
    eval_expr (Var "_self")
  | Super(m_name,es) -> sequence (List.map eval_expr es) >>= fun args ->
