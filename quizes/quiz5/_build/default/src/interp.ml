...
and
  ...
  | Proj(e,id) ->
    ...
    (match List.assoc_opt id fs with
      ...
      | Some (is_mutable, ev) -> if is_mutable then (int_of_refVal ev) >>= fun l ->   (match Store.deref g_store l with
        | None -> error "index out of bounds"
        | Some ev1 -> return ev1) else return ev
      )
  | SetField(e1,id,e2) ->
    eval_expr e1 >>=
    fields_of_recordVal >>= fun fs ->
    (match List.assoc_opt id fs with
      | None -> error "Not Found"
      | Some (is_mutable, ev) -> if is_mutable then (int_of_refVal ev) >>= fun l ->  eval_expr e2 >>= fun ev1 ->
        (match Store.set_ref g_store l ev1 with
        | None -> error "index out of bounds"
        | Some _ -> return UnitVal)  else error "Field not mutable")
  | Unit -> return UnitVal
  | Debug(_e) ->
