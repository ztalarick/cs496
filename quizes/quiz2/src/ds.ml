...
let help : exp_val result -> exp_val = fun r ->
  match r with
  | Ok exp -> exp
  | Error err -> failwith err

let seq_h : exp_val ea_result -> exp_val = fun e -> help (run e)

let rec sequence : (exp_val ea_result) list -> exp_val list = fun ls ->
  match ls with
  | [] -> []
  | h::t -> (seq_h h)::sequence t
