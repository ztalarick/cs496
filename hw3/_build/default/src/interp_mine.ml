...
and
 eval_expr : expr -> exp_val ea_result = fun e ->
  ...
  | Abs(e1)      ->
    eval_expr e1 >>=
    int_of_numVal >>= fun n ->
    if n > 0 then return @@ NumVal n else return @@ NumVal (n * -1)
  | Cons(e1, e2) ->
    eval_expr e1 >>= fun n1 ->
    eval_expr e2 >>=
  ...
  | Hd(e1) ->
    ...
    if n = [] then error "Cannot find the head of an empty list." else return @@ (List.hd n)
  | Tl(e1) ->
  eval_expr e1 >>=
  list_of_listVal >>= fun n ->
  if n = [] then error "Cannot find the tail of an empty list." else return @@ ListVal (List.tl n)
  | Empty(e1) ->
    eval_expr e1 >>=
    list_of_listVal >>= fun n ->
    return @@ BoolVal (n = [])
  ...
  | CaseT(target,emptycase,id1,id2,id3,nodecase) ->
    ...
    | Ds.Node(d, l, r) ->    extend_env id1 d >>+
